    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "RikanPromptRelayMultiLoraGate") {
            ...
            const syncData = (node) => {
                ...
                const loras = [];
                // Hanya memproses widget lora yang benar-benar eksis
                const loraWidgets = node.widgets.filter(w => w.name && w.name.startsWith("lora_"));
                
                loraWidgets.forEach(lw => {
                    const idx = lw.name.split("_")[1];
                    const wToggle = node.widgets.find(w => w.name === `enable_${idx}`);
                    const wSeg = node.widgets.find(w => w.name === `segment_${idx}`);
                    const wMod = node.widgets.find(w => w.name === `modelStr_${idx}`);

                    if (wToggle && wSeg && wMod) {
                        loras.push({
                            enable: wToggle.value,
                            name: lw.value,
                            segment: parseInt(wSeg.value) || 0,
                            modelStr: parseFloat(wMod.value) || 0.0
                        });
                    }
                });
            ...
            const addLoraRow = function(node, index, loraName = "None") {
                ...
                // Segment default SELALU 0, dengan step 10 agar klik tombol panah bernilai +1
                const wSeg = node.addWidget("number", `segment_${index}`, 0, () => syncData(node), { min: 0, max: 99, step: 10, precision: 0 });
                // Strength default SELALU 0.80, dengan step 0.1 agar klik tombol panah bernilai +0.01
